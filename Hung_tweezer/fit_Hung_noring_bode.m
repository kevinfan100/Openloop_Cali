%% ========================================================================
%  Hung_tweezer (No Ring) Transfer Function Fitting with Bode Plot
%  讀取診斷後的頻域數據，執行二階擬合，繪製 Model_6_6 樣式的波德圖
% ========================================================================
%
% Purpose:
%   1. 讀取 diagnose_Hung_noring_data_summary.m 產生的 CSV
%   2. 執行二階轉移函數擬合：H(s) = b / (s² + a1·s + a2)
%   3. 繪製 Model_6_6_Continuous_Weighted.m 樣式的波德圖
%
% Input:
%   results_noring/diagnostics/Raw_Bode_Data.csv
%
% Output:
%   - Console: 擬合參數和品質指標
...
fprintf('\n');
fprintf('========================================================================\n');
fprintf('  Hung_tweezer (No Ring) Transfer Function Fitting\n');
fprintf('========================================================================\n\n');

% --- Path Configuration ---
project_root = fileparts(mfilename('fullpath'));  % Hung_tweezer/
functions_path = fullfile(project_root, '..', 'functions');
addpath(functions_path);

% --- Input File ---
input_file = fullfile(project_root, 'results_noring', 'diagnostics', 'Raw_Bode_Data.csv');
use_csv = true;
...
if ~exist(input_file, 'file')
    error('Input file not found: %s\nRun diagnose_Hung_noring_data_summary.m first.', input_file);
end
...
if PLOT_FIGURE
    ...
    [~, min_freq_idx] = min(frequencies);
    H_ref_measured = magnitudes_linear(min_freq_idx);
    freq_ref = frequencies(min_freq_idx);

    % Tab 1: Normalize by fitted DC gain
    H_model_norm_tab1 = H_model_smooth / dc_gain;
    h_k_norm_tab1 = magnitudes_linear / dc_gain;
    h_db_norm_tab1 = 20*log10(h_k_norm_tab1);

    % Tab 2: Normalize by measured H(0.1 Hz)
    ...
    fig = figure('Name', 'Bode Plot - Hung (No Ring)', 'Position', figure_size);
    ...
    y_min = min(real(h_db_norm_tab1)) - 5;
    ylim([y_min, 5]);
    ...
    % === Title ===
    sgtitle(sprintf('Hung\\_tweezer (No Ring), p=%.1f, \\omega_c=%.1f Hz', p_weight, wc_Hz), ...
        'FontWeight', 'bold', 'FontSize', 24);

    fprintf('  Bode plot generated.\n');

    if SAVE_FIGURE
        output_file = fullfile(project_root, 'results_noring', 'figures', 'Bode_Hung_noring_Model66_style.png');
    ...
    fig_dir = fullfile(project_root, 'results_noring', 'figures');
    if ~exist(fig_dir, 'dir'), mkdir(fig_dir); end
    tabgp.SelectedTab = tab2;
    drawnow;
    output_file2 = fullfile(fig_dir, 'Bode_Hung_noring_DataOnly.png');
