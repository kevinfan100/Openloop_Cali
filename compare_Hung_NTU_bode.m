%% ========================================================================
%  Hung vs NTU Bode Plot Comparison
%  Compare measured frequency response data (normalized by H(0.1 Hz))
% ========================================================================
...
ntu_mag = ntu_table.Magnitude_Linear;
ntu_phase_raw = ntu_table.Phase_deg;

% Normalize magnitude by H(0.1 Hz)
[~, ntu_min_idx] = min(ntu_freq);
ntu_H_ref = ntu_mag(ntu_min_idx);
ntu_mag_norm = ntu_mag / ntu_H_ref;
...
fprintf('NTU:  H(0.1 Hz) = %.4f [V/V], Phase offset = %.2f deg\n', ntu_H_ref, ntu_phase_offset);
...
fig = figure('Name', 'Hung vs NTU Comparison', 'Position', figure_size);
...
semilogx(ntu_freq, ntu_mag_dB, 's-r', 'LineWidth', 3.5, 'MarkerSize', 12, ...
    'MarkerFaceColor', 'none', ...
    'DisplayName', sprintf('NTU (H(0.1 Hz)=%.4f)', ntu_H_ref));
...
%% Save figure
output_file = 'Comparison_Hung_NTU_Bode.png';
exportgraphics(fig, output_file, 'Resolution', 150);
fprintf('\nComparison plot saved: %s\n', output_file);
